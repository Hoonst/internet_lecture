...
def get(url, verify_ssl=True):
    ...
    while True:
        ...
        try:
            ...
            logger.debug(
                "Error occurred during fetching %s", url, exc_info=exc,
            )
            ...
            if attempt == settings.HTTP_RETRIES:
                ...
                logger.debug(
                    "Sleeping for %s seconds", settings.HTTP_DELAY,
                )
...
def load(use_cache_server=True, verify_ssl=True):
    ...
    try:
        ...
        try:
            ret = json.loads(
                get(settings.CACHE_SERVER, verify_ssl=verify_ssl,).decode("utf-8")
            )
