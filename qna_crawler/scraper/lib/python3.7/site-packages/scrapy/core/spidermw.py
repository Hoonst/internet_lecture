...
class SpiderMiddlewareManager(MiddlewareManager):
    ...
    def _add_middleware(self, mw):
        ...
        self.methods["process_spider_output"].appendleft(
            getattr(mw, "process_spider_output", None)
        )
        self.methods["process_spider_exception"].appendleft(
            getattr(mw, "process_spider_exception", None)
        )
    ...
    def scrape_response(self, scrape_func, response, request, spider):
                ...
                try:
                    ...
                    if result is not None:
                        raise _InvalidOutput(
                            "Middleware {} must return None or raise an exception, got {}".format(
        ...
        def process_spider_output(result, start_index=0):
            ...
            for method_index, method in enumerate(method_list, start=start_index):
                ...
                if _isiterable(result):
                    ...
                    raise _InvalidOutput(
                        "Middleware {} must return an iterable, got {}".format(
                            fname(method), type(result)
                        )
                    )
        ...
        dfd.addCallbacks(
            callback=process_spider_output, errback=process_spider_exception
        )
        return dfd
