...
class BaseSgmlLinkExtractor(SGMLParser):
    ...
    def _extract_links(
        self, response_text, response_url, response_encoding, base_url=None
    ):
        ...
        if base_url is None:
            base_url = (
                urljoin(response_url, self.base_url) if self.base_url else response_url
            )
        for link in self.links:
            ...
            link.text = to_unicode(
                link.text, response_encoding, errors="replace"
            ).strip()
            ret.append(link)
        ...
        if self.scan_tag(tag):
            for attr, value in attrs:
                if self.scan_attr(attr):
                    ...
                    if url is not None:
                        link = Link(
                            url=url, nofollow=rel_has_nofollow(dict(attrs).get("rel"))
                        )
...
class SgmlLinkExtractor(FilteringLinkExtractor):
    ...
    def extract_links(self, response):
        ...
        if self.restrict_xpaths:
            ...
            body = u"".join(
                f for x in self.restrict_xpaths for f in response.xpath(x).getall()
